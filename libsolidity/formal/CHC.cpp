...
{
	SMTEncoder::endVisit(_range);

	auto prevArray = dynamic_pointer_cast<SymbolicArrayVariable>(m_context.expression(_range.baseExpression()));
	auto sliceArray = dynamic_pointer_cast<SymbolicArrayVariable>(m_context.expression(_range));
	solAssert(prevArray && sliceArray, "");
	...
	auto end = _range.endExpression() ? expr(*_range.endExpression()) : prevArray->length();
