...
)
{
	...
	// Bases 0 and 1. There is no overflow. The optimizer can simplify the `exp` for these cases.
	...
	// Base -1. There is no overflow
	if (_baseType.isNegative() && (u2s(baseValue) == -1))
		return m_functionCollector.createFunction(functionName, [&]() {
			return Whiskers(R"(
				function <functionName>(exponent) -> power {
					exponent := <exponentCleanupFunction>(exponent)
					switch and(exponent, 1)
					case 0 { power := 1 }
					default { power := <max> }
				}
				)")
	...
	if (_baseType.isNegative())
	{
		...
		auto power = boost::multiprecision::pow(bigint(signedValue), exponentUpperbound);
		auto overflowedPower = boost::multiprecision::pow(bigint(signedValue), exponentUpperbound + 1);
